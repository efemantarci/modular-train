...
class Temporal3DProbabilityPredictor(nn.Module):
    # Predicts a probability distribution over 3D space
    def __init__(self, spatial_bins=20, hidden_dim=512):
        super(Temporal3DProbabilityPredictor, self).__init__()
        ...
        # LSTM to handle temporal sequence information
        self.lstm = nn.LSTM(
            input_size=128 * (spatial_bins // 8) ** 3, 
            hidden_size=hidden_dim, num_layers=2, batch_first=True)
        
        # Fully connected layers
        self.fc1 = nn.Linear(hidden_dim, 512)
        self.fc2 = nn.Linear(512, spatial_bins ** 3)  # Output a probability for each voxel

    def process_time_step(self, x_t):
        x_t = nn.functional.relu(self.conv1(x_t))  
        x_t = self.pool1(x_t)                      
        x_t = nn.functional.relu(self.conv2(x_t))
        x_t = self.pool2(x_t)
        x_t = nn.functional.relu(self.conv3(x_t))
    ...
    def forward(self, x):
        ...
        # Fully connected layers
        x = nn.functional.relu(self.fc1(lstm_out))
        x = self.fc2(x)  # Shape: (batch_size, spatial_bins ** 3)

        # Reshape to spatial grid and apply softmax to get probabilities
        x = x.view(batch_size, self.spatial_bins, self.spatial_bins, self.spatial_bins)
        x = nn.functional.softmax(x.view(batch_size, -1), dim=1)
        x = x.view(batch_size, self.spatial_bins, self.spatial_bins, self.spatial_bins)
        
        return x  # Output: probability distribution over spatial grid
